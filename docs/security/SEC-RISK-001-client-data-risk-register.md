...
| **Version** | 1.0 |
| **Status** | Draft — risks identified, not yet assessed or accepted |
...
The risks below are **identified from how the business actually operates**. Scores are proposed starting points for discussion — they have not been assessed or accepted by the owner, and the Treatment column is deliberately blank.
...
| R-03 | Admin password brute-forced | `/api/auth/login` has no rate limiting; password strength not verified | Full access to all client data, invoices, and CRM records | 3 | 5 | 15 | | | Open |
...
**R-03** is concrete and fixable: the same Workers rate-limit binding already protecting `/api/contact` could be applied to `/api/auth/login`.
